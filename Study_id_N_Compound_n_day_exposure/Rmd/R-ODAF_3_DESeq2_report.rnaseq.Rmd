...
params:
  ...
  species: "zebrafish"                # one of human, mouse, rat, hamster, zebrafish, rainbowtrout, chinesemedaka, fatheadminnow
  ...
  cpus: !expr as.numeric(Sys.getenv('SLURM_CPUS_PER_TASK')) # Set to a lower number (e.g., 2 to 4) if you aren't working in a server environment
...
title: "`r paste('Transcriptome analysis:', gsub(pattern = '_', replacement = ' ', x = Sys.getenv('STUDY_ID_DIR')))`"
author: "Jory Curry"
---
...
params <- list()
params$projectdir="/mnt/Study_id_50_PFBS_4.83_day_exposure"
params$project_name="Study_id_50_PFBS_4.83_day_exposure"
params$species="zebrafish"
params$dose="Dose"
params$design="Dose"
params$intgroup=c("Dose")
params$nmr_threshold=3000000
...
paths$processed <- normalizePath(file.path(paths$output, "Trimmed_reads", "RSEM")) #This directory should be contain the count table output by R-ODAF_1_sequencing_DataPreprocess...
...
```{r create_annotations_locally_on_machine_with_internet, eval=F}
# library('AnnotationHub')
# library('biomaRt')
...
# ##################
# fatheadminnow
# ensembl_species <- "ppromelas_gene_ensembl"
# species_gene_symbol <- "external_gene_name"
# #############################################
# 
# species <- if_else(
...
# 
# if (species %in% c("zebrafish", "rainbowtrout", "hamster", "rat", "mouse", "human")) {
#   ############################################################
...
#     # Path to your local GFF/GTF file
#     gtf_file <- "C:/Users/curryj/OneDrive - EC-EC/Desktop/tPOD_Curation_Project/tPOD_curation/annotationfiles/Oncorhynchus_mykiss.USDA_OmykA_1.1.112.gtf"
...
#   } else if (species == "rainbowtrout") {
#     readr::write_csv(id_table_entrez, "C:/Users/curryj/OneDrive - EC-EC/Desktop/tPOD_Curation_Project/tPOD_curation/annotationfiles/rainbowtrout_annotated_id_table_entrez_wholegenome.csv") 
#   } else if (species == "mouse") {
...
#   
# } else if (species %in% c("fatheadminnow")) {
#   ######################
#   ### Fathead Minnow ###
#   ######################
#   
#   #No BioMart is available so all information is strictly from the .gtf file
#   
#   # Path to your local GFF/GTF file
#   gtf_file <- "C:/Users/curryj/OneDrive - EC-EC/Desktop/tPOD_Curation_Project/tPOD_curation/annotationfiles/FHM_genomic.gtf"
#   
#   # Import the GFF/GTF file
#   annotations <- import(gtf_file)
#   
...
#     dplyr::select(-product.x, -product.y)
#   
#   # Filter for genes and select relevant columns
#   gene_annotations_df <- annotations_df %>%
#     dplyr::filter(type == "gene") %>%
#     dplyr::select(seqnames, start, end, strand, gene_id, db_xref, product)
#   
...
#     dplyr::select(ensembl_gene_id, external_gene_name, description, entrezgene_id, entrezgene_accession)
#   
#   readr::write_csv(annotations_df, "C:/Users/curryj/OneDrive - EC-EC/Desktop/tPOD_Curation_Project/tPOD_curation/annotationfiles/fatheadminnow_annotated_id_table_entrez_wholegenome.csv") 
#   
# }
...
#### THIS SECTION AND FOLLOWING SECTIONS NEED TO BE UPDATED FOR FATHEAD MINNNOW!

annotate_deseq_table <-
  function(deseq_results_list,
    ...
    for (i in seq_along(deseq_results_list)) {
      ...
      deg_table <-
        cbind(
          genes = row.names(deg_table),
          as.data.frame(deg_table),
          contrast = gsub(
            paste0("log2.*", DESIGN, " "),
            "",
            deg_table@elementMetadata[[2]][2]
          )
        )
      #dummy_contrast <- unique(deg_table$contrast)
      names(deg_table)[names(deg_table) == "genes"] <- join_by
      deg_table <-
        dplyr::left_join(deg_table, id_table, by = join_by)
      if (nrow(deg_table) == 0) {
        print("Warning: Join resulted in an empty dataframe!")
      }
      deg_table <-
        dplyr::mutate(deg_table,
                      linearFoldChange = if_else(
                        as.numeric(log2FoldChange) > 0,
                        2 ^ as.numeric(log2FoldChange),-1 / (2 ^ as.numeric(log2FoldChange))
                      ))
      deg_table <-
        if (Platform == "RNA-Seq") {
          dplyr::select(
            deg_table,!!sym(biomart_filter),!!sym(species_gene_symbol),
            description,
            baseMean,
            log2FoldChange,
            linearFoldChange,
          ...
          dplyr::select(
            deg_table,!!sym(biomart_filter),!!sym(species_gene_symbol),!!sym(join_by),
            description,
            baseMean,
            log2FoldChange,
            linearFoldChange,
      ...
      if (filter_results &&
          "linearFoldChange" %in% colnames(deg_table) &&
          nrow(deg_table) > 0) {
        deg_table <-
          dplyr::filter(deg_table,!is.na(padj) &
                          padj < alpha &
                          abs(linearFoldChange) > linear_fc_filter)
      }
      if (nrow(deg_table) > 0) {
        annotated_results[[i]] <- distinct(deg_table)
      }
    }
    if (all_empty) {
      print("All DESeq2 results were empty. Returning dummy result.")
      dummy_contrast <- gsub(paste0("log2.*", DESIGN, " "), "", deg_table@elementMetadata[[2]][2])
      if (Platform == "RNA-Seq") {
        # Define a dummy result
        ...
        names(dummy_result)[1:2] <-
          c(biomart_filter, species_gene_symbol)
      } else if (Platform == "TempO-Seq") {
        # Define a dummy result
        dummy_result <- data.frame(
          temp_col1 = "DUMMY",
        ...
        names(dummy_result)[1:3] <-
          c(biomart_filter, species_gene_symbol, join_by)
      }
      return(dummy_result)
    }
    return(data.table::rbindlist(annotated_results))
  }

# Ensure alpha and linear_fc_filter are set correctly
print(alpha)
alpha <- params$fdr_threshold
print(linear_fc_filter)
linear_fc_filter <- params$linear_fc_filter

# Write dataframe of all results
# Significant only
significantResults <- annotate_deseq_table(resList, filter_results = T, join_by = if_else(condition = Platform == "TempO-Seq", true = "PROBE", false = if_else(condition = species == "fatheadminnow", true = species_gene_symbol, false = biomart_filter)))
significantResults <- significantResults %>% dplyr::mutate(contrast = as.factor(contrast))

print(paste("Total number of significant results:", nrow(significantResults)))

# Set 'anyDEGs' to specify what should be evaluated if there are no DEGs to display in the report
if (any(significantResults[[species_gene_symbol]] == "DUMMY", na.rm = TRUE)) {
  print(paste0("significantResults is empty."))
  anyDEGs <- FALSE # For setting evaluation logic
} else {
  anyDEGs <- TRUE # For setting evaluation logic
}

# All results including non-significant
allResults <- annotate_deseq_table(resListAll, filter_results = F, join_by = if_else(condition = Platform == "TempO-Seq", true = "PROBE", false = if_else(condition = species == "fatheadminnow", true = species_gene_symbol, false = biomart_filter)))
...
if (Platform == "RNA-Seq") {
	...
	allResults[[species_gene_symbol]][is.na(allResults[[species_gene_symbol]])] <- allResults[["PROBE"]][is.na(allResults[[species_gene_symbol]])] 
	# Replace blank gene symbols with PROBE ID
	allResults[[species_gene_symbol]][allResults[[species_gene_symbol]] == ""]  <- allResults[["PROBE"]][allResults[[species_gene_symbol]] == ""]
}
...
degTable <- significantResults %>%
  ...
  dplyr::count() %>%
  dplyr::mutate(
    # Extract numeric part from contrast using sub and convert to numeric
    sort_key = as.numeric(sub(" .*", "", contrast))
  ) %>%
  # Arrange rows based on the numeric sort_key in ascending order
  dplyr::arrange(sort_key) %>%
  dplyr::select(-sort_key)  # Remove the temporary sorting key column
...
if (all_zero) {
  summaryTable <- data.frame( genes =  "DUMMY",
                            baseMean =  0)
} else {
  summaryTable <- data.frame( genes =  row.names(resList[[longest]]),
                            baseMean = resList[[longest]]$baseMean )
}
col_name <- if_else(condition = Platform == "RNA-Seq", true = biomart_filter, false = "PROBE")
...
for (i in 1:length(resList)) {
  ...
  setnames(toJoin, 1, col_name)
  toJoin <- dplyr::mutate(toJoin, linearFoldChange = ifelse(log2FoldChange > 0,
                                                2 ^ log2FoldChange,
                                                -1 / (2 ^ log2FoldChange)))
  toJoin <- toJoin[, c(1:3, 7, 4:6)]
  summaryTable <- dplyr::left_join(summaryTable, dplyr::select(toJoin, !c(baseMean, pvalue, lfcSE)), by = col_name)
...
# Ensure significantResults is not empty
#if (length(unique(significantResults[[biomart_filter]])) == 0) {
#  stop("No significant results found.")
#}

# Subset the rld object
if (Platform == "RNA-Seq") {
  if (species == "fatheadminnow") {
...
# Check if rld_degs has non-zero dimensions
if (nrow(rld_degs) == 0 || ncol(rld_degs) == 0) {
  stop("rld_degs has zero rows or columns. Check your significantResults and rld object.")
}
...
if (Platform == "RNA-Seq") {
  if (species == "fatheadminnow") {
    	mat <- assay(rld)[row.names(assay(rld)) %in%
                                          significantResults[[species_gene_symbol]],]
  } else {
    	mat <- assay(rld)[row.names(assay(rld)) %in%
...
#knitr::include_graphics(path = normalizePath(file.path(paths$DEG_output, "plots", "Heatmap_norm_counts_vs._logFC_sig_genes.pdf")), error = FALSE, rel_path = FALSE, out.width='100%', out.height='100%')
# color = inferno(10)
pheatmap(mat,
         annotation_col = heatmap_df,
         show_rownames = FALSE,
...
if (Platform == "RNA-Seq") {
  if (species == "fatheadminnow") {
    mat_top <- assay(rld)[row.names(assay(rld)) %in% allResultsOrdered_logFC_filter[1:nHeatmapDEGs,][[species_gene_symbol]],]
	...
	mat_top <- assay(rld)[row.names(assay(rld)) %in% allResultsOrdered_logFC_filter[1:nHeatmapDEGs,][["PROBE"]],]
	mat_top <- mat_top - rowMeans(mat_top)
	genes_for_heatmap <- data.frame(genes = row.names(mat_top))
...
#knitr::include_graphics(path = normalizePath(file.path(paths$DEG_output, "plots", "Heatmap_norm_counts_vs._logFC_top_genes_byFC.pdf")), error = FALSE, rel_path = FALSE, out.width='100%', out.height='100%')
pheatmap(mat_top,
         annotation_col = heatmap_df,
         labels_row = genes_for_heatmap,
         show_rownames = TRUE,
...
if (Platform == "TempO-Seq") {
	id_table <- id_table %>%
  		mutate(!!sym(biomart_filter) := if_else(is.na(!!sym(biomart_filter)) & is.na(!!sym(species_gene_symbol)), PROBE, !!sym(biomart_filter)))
}
