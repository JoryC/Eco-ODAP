...
params:
  ...
  chemical_name: "MyChemicalName" #This should match the name of the chemical/compound/treatment in the metadata
  ...
  species: "species" #One of zebrafish, mouse, rat, hamster, dog, fruitfly, human, fatheadminnow, chinesemedaka, rainbowtrout, daphnia... or anything else for generic species. If generic species, use "Generic" in bmdx_platform_id.
  bmdx_platform_id: "MyPlatformName" #See file bmdx_platforms for list of possible platforms or open the BMDExpress GUI, select File -> Update Annotations to see list of annotation IDs and pull updated annotations... If species is chinese medaka, rainbowtrout or fatheadminnow use danRer11_Ensembl... If daphnia use dm6_Ensembl (drosophilla). For generic species and no pathway analysis, use "Generic"
  ...
  filter_low: TRUE #TRUE or FALSE, do you want to filter BMDs lower than the lowest dose/10?
  filter_high: TRUE #TRUE or FALSE, do you want to filter BMDs higher than the highest dose?
  turn_off_BMD_filtering: FALSE #TRUE or FALSE, do you want to turn off BMD filtering? This is useful for testing purposes.
  categoryP: 0.05 # Pvalue for filtering categories using Fisher.s.Exact.Two.Tail... default 0.05. If onerepperdose, set to 1.
  ...
  executeBMDExpress3: TRUE #TRUE or FALSE
  threads: 16 #Number of threads to use for BMDExpress
...
genericspecies <- !params$species %in% c("chinesemedaka", "fatheadminnow", "rainbowtrout", "daphnia","zebrafish", "mouse", "rat", "human", "hamster", "dog", "fruitfly")
envspecies <- species %in% c("chinesemedaka", "fatheadminnow", "rainbowtrout", "daphnia")
...
if (params$species == "chinesemedaka") {
  ...
  file <-
    "C:/Users/curryj/EC-EC/S&T Labo MolTox=S&T MolTox Lab [Unclassified] - 2021-MSABS/Jory/20241_12_Multspecies_Project/FatheadMinnow/genome/fatheadminnow_annotated_id_table_entrez_wholegenome.csv"
} else if (params$species == "daphnia") {
  file <-
    "C:/Users/curryj/EC-EC/S&T Labo MolTox=S&T MolTox Lab [Unclassified] - 2021-MSABS/Jory/20241_12_Multspecies_Project/Daphnia/genome/daphnia_annotated_id_table_entrez_wholegenome.csv"
} else {
  stop("Species not supported for homologous gene annotation.")
...
if (params$species %in% c("chinesemedaka", "rainbowtrout")) {
  ...
  homolog <-
    left_join( bmd_express_input[-1,], annotation[, c("SampleID", "dmelanogaster_eg_homolog_ensembl_gene")]) %>% #join the bmd_express_input data with the annotation data
    distinct() %>%
    group_by(SampleID) %>%
    dplyr::filter(!(is.na(dmelanogaster_eg_homolog_ensembl_gene) & n() > 1)) %>% #Filter out those pesky NAs for genes with multiple homologs
    ungroup()
  ...
  homologous_genes_table_output <- homolog %>%
    dplyr::group_by(SampleID) %>%
    tally() %>%
    dplyr::rename(n_homologous_dm_genes = n)
  ...
  homologous_genes_table_output <-
    ...
    dplyr::rename(
      !!paste0(params$species, "_ensembl_gene_id") := SampleID,
      !!paste0(params$species, "_gene_name") := external_gene_name,
      homologous_fruitfly_ensembl_gene_id = dmelanogaster_eg_homolog_ensembl_gene,
      homologous_fruitfly_external_gene_name = dmelanogaster_eg_homolog_associated_gene_name
    ) %>%
    dplyr::select(
      !!paste0(params$species, "_ensembl_gene_id"),
      !!paste0(params$species, "_gene_name"),
      n_homologous_dm_genes,
      homologous_fruitfly_ensembl_gene_id,
      homologous_fruitfly_external_gene_name
    ) %>%
    distinct()
  
  # Calculate the number of rows with NA in the homologous_zebrafish_ensembl_gene_id column
  num_missing_homologs <- sum(is.na(homologous_genes_table_output$homologous_fruitfly_ensembl_gene_id))
  
  
  new_row <- bmd_express_input[1,]
  new_row$dmelanogaster_eg_homolog_ensembl_gene <- "Dose"
  homolog <- rbind(new_row, homolog) #add the dose metadata row back
  homolog <-
    homolog[, c(colnames(homolog)[length(colnames(homolog))], colnames(homolog)[2:(length(colnames(homolog)) - 1)], colnames(homolog)[1])] #swap the first and last column
  homolog <- homolog[,-ncol(homolog)] #remove the old ensemble ids
  ...
  write_tsv(
    ...
    file = file.path(
      ...
      if_else(
        ...
        false = "bmdexpress_input_log2_transformed_dm_homologs.txt"
      )
    )
  )
  ...
  print(
    paste(
      params$species,
      "Gene IDs/probes were annotated with homologous fruitfly ensemble ids to facilitate the category analysis"
    )
  )
  # print(paste(
  #   length(entrez$SampleID[is.na(entrez$SampleID)]),
  ...
  # Print the total number of genes with multiple homologous fruitfly genes
  print(paste("There were", n_multihomolg_genes, "genes/probes with multiple homologous fruitfly genes."))

  # Print a breakdown of the number of entries for each gene with more than one homologous gene
  cat("Breakdown of", params$species, "genes with more than one homologous fruitfly gene:\n")
  for (i in names(n_multihomolg_genes_table)) {
    if (as.numeric(i) >= 2) {
      cat("Number of genes ids/probes with", i, "homologous genes:", n_multihomolg_genes_table[i], "\n")
    }
  ...
  #Print the total number of genes without homologs
  print(paste0("And there were ", num_missing_homologs, " ", params$species, " gene IDs/probes without a corresponding homologous fruitfly ensembl gene id. The count values were retained and the probe was renamed to NA"))
  
  #Compare the number of genes before and after adding homologs
  print(paste0("After annotating ", params$species, " gene ids with homologous fruitfly ensembl gene ids, were any of the original ", params$species, " gene ids lost?"))
  check <- !all(homologous_genes_table_output[,1][[1]] %in% bmd_express_input$SampleID[-1]) #all entrez gene accessions in sampleId?
  print(paste(check))
  if (check == TRUE) {
    print(paste(length(homologous_genes_table_output[,1][[1]][which(!(homologous_genes_table_output[,1][[1]] %in% bmd_express_input$SampleID[-1]))]), " Gene ids lost:"))
...
If the species analyzed was chinese medaka, fathead minnow rainbowtrout, or daphnia the table of all homologous zebrafish/fruitfly genes after conducting a BLASTN or Biomart query will appear here. These homologous genes are used for the category analysis only:

```{r homolog_tables, eval = envspecies}
if (params$species %in% c("chinesemedaka", "fatheadminnow", "rainbowtrout")) {
...
DT::datatable(
  ...
  homologous_genes_table_output <- homologous_genes_table_output %>%
  dplyr::mutate(n_homologous_dm_genes = if_else(condition = is.na(homologous_fruitfly_ensembl_gene_id), true = 0, false = n_homologous_dm_genes))

# Render the table using DT with filtering, sorting, and download options
DT::datatable(
  homologous_genes_table_output,
  rownames = FALSE,
  colnames = c("Gene id", "Gene name", "Number of homologous fruitfly genes", "Homologous gene ensembl id", "Homologous gene name"),
  extensions = 'Buttons',
  options = list(
    dom = 'Bfrtip',  # Show buttons and filter/search bar
    buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),  # Add download buttons
  ...
  homolog <- read_tsv(
    file.path(
      ...
      if_else(
        condition = append_chemname,
        true = if_else(
          condition = params$species %in% c("chinesemedaka", "fatheadminnow", "rainbowtrout"), 
          true = paste0("bmdexpress_input_log2_transformed_zf_homologs_", params$chemical_name, ".txt"),
          false = paste0("bmdexpress_input_log2_transformed_dm_homologs_", params$chemical_name, ".txt")
        ),
        false = if_else(
          condition = params$species %in% c("chinesemedaka", "fatheadminnow", "rainbowtrout"),
          true = "bmdexpress_input_log2_transformed_zf_homologs.txt",
          false = "bmdexpress_input_log2_transformed_dm_homologs.txt"
        )
      )
    )
...
if (params$turn_off_BMD_filtering) {
  if (params$onerepperdose) {
    json_content <- list(
      ...
      categoryAnalysisConfigs = list(
        list(
          ...
          removePromiscuousProbes = FALSE,
          removeBMDGreaterHighDose = FALSE,
          bmdPValueCutoff = 0,
          #Floor the BMD value cutoff because they will all be zero
        ...
        list(
          ...
          maxFoldChange = NULL,
          prefilterPValueMin = NULL,
          #Floor the BMD p value cutoff because they will all be zero
          deduplicateGeneSets = TRUE,
          identifyConflictingProbeSets = FALSE,
          correlationCutoffForConflictingProbeSets = NULL,
          minGenesInSet = params$min_gene,
...
```{r BMDExpress3_configuration_non-supported_species_BMDs_only, echo=FALSE, warning=FALSE, eval=envspecies | genericspecies}
#BMDExpress3 natively supports these species: c("zebrafish", "mouse", "rat", "human", "hamster", "dog", "fruitfly")
# Define the JSON content as an R object
if (params$onerepperdose) {
json_content <- list(
  ...
  if (params$onerepperdose) {
    json_content <- list(
      ...
      bm2FileName = normalizePath(file.path(
        ...
        "BMDExpressData",
        if_else(
          condition = params$species %in% c("chinesemedaka", "rainbowtrout", "fatheadminnow"),
          true = paste0(
          "BMDExpressFile",
          params$chemical_name,
          "_zf_homologs.bm2"
            ...
            file.path(
              ...
              if_else(
                ...
                true = paste0(
                  ...
                  false = paste0(
                    "bmdexpress_input_log2_transformed_dm_homologs_",
                    params$chemical_name,
                    ".txt"
                  )
                ),
                false = if_else(
                  condition = params$species %in% c("chinesemedaka", "rainbowtrout", "fatheadminnow"),
                  true = paste0("bmdexpress_input_log2_transformed_zf_homologs_.txt"),
                  false = paste0("bmdexpress_input_log2_transformed_dm_homologs_.txt")
                )
              )
            )
          ),
...
if (params$species %in% c("chinesemedaka", "rainbowtrout", "fatheadminnow")) {
# Specify the file path
config_file_path_zf_homolog <-
    normalizePath(file.path(
      directory_path,
      "BMDExpressData",
      paste0("config", params$chemical_name, "_zf_homologs.json")
    ))
...
cat("JSON data has been saved to", config_file_path_zf_homolog)
  ...
  # Specify the file path
  config_file_path_dm_homolog <-
    normalizePath(file.path(
      directory_path,
      "BMDExpressData",
      paste0("config", params$chemical_name, "_dm_homologs.json")
    ))
  
  # Write the JSON data to a file
  write(json_data, file = config_file_path_dm_homolog)
  
  # Print a message indicating the file has been saved
  cat("JSON data has been saved to", config_file_path_dm_homolog)
}
...
config_file_path_homolog <- if_else(
  condition = params$species %in% c("fatheadminnow", "rainbowtrout", "chinesemedaka"),
  true = config_file_path_zf_homolog,
  false = config_file_path_dm_homolog
)

# Second, analyze the data
if (os == "Windows") {
  # Escape the path to handle spaces
  config_file_path_homolog <- paste0("\"", config_file_path_homolog, "\"")
  # Defining the command
  cmd <- paste0('powershell -Command "bmdexpress3-cmd analyze --config-file \"', config_file_path_homolog, '\""')
  # Execute the command
  analyze <- system(cmd)
  ...
  config_file_path_homolog <- paste0("'", config_file_path_homolog, "'")
  # Defining the command
  cmd <- paste0("bash -c 'bmdexpress3-cmd analyze --config-file ", config_file_path_homolog, "'")
  # Execute the command
  analyze <- system(cmd)
...
if (params$species %in% c("fatheadminnow", "rainbowtrout", "chinesemedaka")) {
  ...
  input_bm2_dm_homologs <-
    normalizePath(file.path(
      directory_path,
      "BMDExpressData",
      paste0("BMDExpressFile", params$chemical_name, "_dm_homologs.bm2")
    ))
}
williamsdata <-
  normalizePath(file.path(
...
if (params$species %in% c("fatheadminnow", "rainbowtrout", "chinesemedaka")) {
  ...
  input_bm2_dm_homologs <- paste0("'", input_bm2_dm_homologs, "'")
}
...
if (params$species %in% c("fatheadminnow", "rainbowtrout", "chinesemedaka")) {
  ...
  categoricaldata_dm_homologs <- paste0("'", categoricaldata_dm_homologs, "'")
}
...
categoryfiles <-
  normalizePath(list.files(
    ...
    pattern = if_else(condition = params$species %in% c("fatheadminnow", "rainbowtrout", "chinesemedaka"), true = paste0(params$chemical_name, "(_zf_homologs)?\\.txt$"), false = paste0(params$chemical_name, "(_dm_homologs)?\\.txt$")),
    full.names = TRUE,
    ignore.case = TRUE
  )) %>% setNames(chemname)
}
...
metadata_table_3 <- metadata_table_2 %>%
   dplyr::filter(!!sym(params$treatment_var) == chemname) %>%
  dplyr::select(Run, Organism, Exposure_duration_days, !!sym(params$treatment_var), !!sym(params$dose), DoseUnits) %>% # You may need to alter this if your metadata has different columnn names
...
raw_data_filtered_results <-
  BMDfiltering(x = raw_data,
               ...
               no_filtering = params$turn_off_BMD_filtering
  )
...
if(params$turn_off_BMD_filtering) {
  # If no filtering is enabled, set all filtered datasets to the original
  filtered_data_1 <- raw_data_filtered_results$initial
  filtered_data_2 <- raw_data_filtered_results$initial
  filtered_data_3 <- raw_data_filtered_results$initial
  filtered_data_4 <- raw_data_filtered_results$initial
  filtered_data_5 <- raw_data_filtered_results$initial
  filtered_data_6 <- raw_data_filtered_results$initial
  
  # All row counts will be the same as initial
  filtered_data_1_rows <- initial_rows
  filtered_data_2_rows <- initial_rows
  filtered_data_3_rows <- initial_rows
  filtered_data_4_rows <- initial_rows
  filtered_data_5_rows <- initial_rows
  filtered_data_6_rows <- initial_rows
  
  # All filter outputs will have the same counts
  filter_BMD_BMDL_rows <- initial_rows
  filter_BMDU_BMDL_rows <- initial_rows
  filter_BMDU_BMD_rows <- initial_rows
  filter_lowdose_rows <- initial_rows
  filter_highdose_rows <- initial_rows
  filter_fitP_rows <- initial_rows
  ...
  # Assuming these variables come from elsewhere in your code
  # If they don't exist, you'll need to calculate them here
...
if (genericspecies) {
  go_raw_data <- NA
  go_raw_data_filtered <- NA
  reactome_raw_data <- NA
  reactome_raw_data_filtered <- NA
}
```

```{r save_allresults, echo = FALSE, warning = FALSE, message = FALSE}
#RData
dir.exists(normalizePath(file.path(directory_path, "Outputs"))) %>% ifelse(TRUE, dir.create(normalizePath(file.path(directory_path, "Outputs"))))
...
```{r filter_results_table_GO, echo=FALSE, warning=FALSE, message=FALSE, eval = !genericspecies}
...
```{r vtree_GO, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=6, eval = !genericspecies}
...
```{r filter_results_table_REACTOME, echo=FALSE, warning=FALSE, message=FALSE, eval = !genericspecies}
...
```{r vtree_reactome, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=6, eval = !genericspecies}
# Reformat the data for vtree
...
if (nrow(accumulation_data) > 0) {
  ...
  # Check if any of the filtering parameters are TRUE
  no_x_limits <- isTRUE(params$turn_off_BMD_filtering) || isTRUE(params$filter_high) || isTRUE(params$filter_low)

  # Create the accumulation plot - start with base elements
  accumulation_plot <- ggplot(accumulation_data, aes(x = logBMD, y = CumulativeCount))
  
  # Conditionally apply x-axis limits based on BMD filtering parameter
  if (no_x_limits) {
    # No x-axis limits when BMD filtering is turned off
    accumulation_plot <- accumulation_plot + scale_x_continuous(oob = function(x, limits) x)
  } else {
    # Apply x-axis limits when BMD filtering is enabled
    accumulation_plot <- accumulation_plot + scale_x_continuous(
      limits = c(
...
if (nrow(raw_data_filtered) > 0) {
  ...
  # Add x-axis scale with conditional limits based on BMD filtering parameter
  if (no_x_limits) {
    # No x-axis limits when BMD filtering is turned off
    histPlots <- histPlots + scale_x_continuous(oob = function(x, limits) x)
  } else {
    # Apply x-axis limits when BMD filtering is enabled
    histPlots <- histPlots + scale_x_continuous(
  ...
  # Continue with the rest of the plot elements
  histPlots <- histPlots +
...
```{r GO_accumulation_plot, echo=FALSE, warnings = TRUE, fig.height = 8, fig.width = 10, out.width = '100%', fig.cap = "Accumulation of Gene Ontology biological processes with at least 3 gene BMDs", eval = !genericspecies}
...
if (nrow(go_raw_data_filtered) > 0) {
  ...
  # Conditionally apply x-axis limits
  if (no_x_limits) {
    # No x-axis limits when any filtering parameter is TRUE
    goPlot <- goPlot + scale_x_continuous(oob = function(x, limits) x)
  } else {
    # Apply standard x-axis limits
    goPlot <- goPlot + scale_x_continuous(
  ...
  goPlot <- goPlot +
    ...
    # labels
    labs(
      title = paste0(chemname, " (n=", nrow(go_raw_data_filtered), " GO terms)"), 
      x = paste0("logBMD ", "(", unique(metadata$DoseUnits), ")"), 
      y = "Cumulative GO terms"
    ) +
    # theme
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5, size = 12))
...
```{r reactome_accumulation_plot, echo=FALSE, warnings = TRUE, fig.height = 8, fig.width = 10, out.width = '100%', fig.cap = "Accumulation of REACTOME pathways with at least 3 gene BMDs", eval = !genericspecies}
...
if (nrow(reactome_raw_data_filtered) > 0) {
  ...
  # Create base plot
  reactomePlots <- ggplot(reactome_raw_data_filtered, aes(x = logBMD, y = rank_count))
  
  # Conditionally apply x-axis limits
  if (no_x_limits) {
    # No x-axis limits when any filtering parameter is TRUE
    reactomePlots <- reactomePlots + scale_x_continuous(oob = function(x, limits) x)
  } else {
    # Apply standard x-axis limits
    reactomePlots <- reactomePlots + scale_x_continuous(
      limits = c(min_x, max_x),
      oob = function(x, limits) x
    )
  }
...
if (!genericspecies) {
  write_csv(go_raw_data_filtered, file = normalizePath(file.path(
    directory_path,
    "Outputs",
    paste0("GO_Data_R-ODAF_DEGs", chemname, ".csv")
  )))
  write_csv(reactome_raw_data_filtered, file = normalizePath(file.path(
    directory_path,
    "Outputs",
    paste0("Reactome_Data_R-ODAF_DEGs", chemname, ".csv")
  )))
  if (nrow(reactome_raw_data_filtered) > 0) {
    ggsave(
      reactomePlots,
      path = normalizePath(file.path(directory_path, "Outputs")),
      filename = paste0("REACTOME_accumulation_plot", chemname, ".pdf")
    )
  }
  if (nrow(go_raw_data_filtered) > 0) {
    ggsave(
      goPlot,
      path = normalizePath(file.path(directory_path, "Outputs")),
      filename = paste0("GO_accumulation_plot", chemname, ".pdf")
    )
  }
}
write_csv(raw_data_filtered, file = normalizePath(file.path(
  directory_path,
  "Outputs",
  paste0("BMD_Data_for_Histogram_R-ODAF_DEGs", chemname, ".csv")
)))
if (nrow(raw_data_filtered) > 0) {
  ggsave(
    histPlots,
    path = normalizePath(file.path(directory_path, "Outputs")),
    filename = paste0("BMD_Histogram", chemname, ".pdf")
  )
}
```
...
if (genericspecies) {
 goBMDvalues <- NA
 reactomeBMDvalues <- NA
}

# Modify the create_summary_plot function with a better warning text positioning
create_summary_plot <- function(df, params, chemname, lowestdoses, highestdoses, metadata, logBMDvalues, 
                               goBMDvalues, reactomeBMDvalues) {
  # Check if any of the filtering parameters are TRUE
  no_x_limits <- isTRUE(params$turn_off_BMD_filtering) || isTRUE(params$filter_high) || isTRUE(params$filter_low)
  
  # Safely calculate x-axis limits with error handling
  tryCatch({
    min_x <- log(((lowestdoses[[params$dose]][lowestdoses[[params$treatment_var]] == chemname]) / 10), 
                base = params$logtransformationscale)
    max_x <- log((highestdoses[[params$dose]][lowestdoses[[params$treatment_var]] == chemname]), 
                base = params$logtransformationscale)
  }, error = function(e) {
    # Default values if there's an error in dose calculations
    warning("Error calculating dose limits, using data range instead: ", e$message)
    min_x <- min(df$lowerCI, na.rm = TRUE) * 0.9  # Use 90% of min value as fallback
    max_x <- max(df$upperCI, na.rm = TRUE) * 1.1  # Use 110% of max value as fallback
  })
  
  # Function to safely get the size of an object (handles various object types)
  get_element_count <- function(obj) {
    if (is.null(obj)) return(0)
    if (is.vector(obj) && !is.list(obj)) return(length(obj))
    if (is.list(obj)) return(length(obj))
    if (is.data.frame(obj)) return(nrow(obj))
    if (is.matrix(obj)) return(nrow(obj))
    return(0)  # Default fallback
  }
  
  # Check if warnings are needed for objects with few elements
  go_count <- get_element_count(goBMDvalues)
  reactome_count <- get_element_count(reactomeBMDvalues)
  
  go_warning <- go_count < 3
  reactome_warning <- reactome_count < 3
  
  # Prepare warning text if needed
  warning_text <- NULL
  if (go_warning || reactome_warning) {
    warning_text <- "WARNING: "
    if (go_warning) {
      warning_text <- paste0(warning_text, "GO terms (n=", go_count, ")")
      if (reactome_warning) warning_text <- paste0(warning_text, " and ")
    }
    if (reactome_warning) {
      warning_text <- paste0(warning_text, "Reactome pathways (n=", reactome_count, ")")
    }
    warning_text <- paste0(warning_text, " < 3. Interpret tPODs with EXTREME CAUTION.")
  }
  ...
  # Conditionally apply x-axis limits
  if (!no_x_limits && !is.na(min_x) && !is.na(max_x)) {
    plot <- plot + scale_x_continuous(limits = c(min_x, max_x))
  }
  
  # Complete the plot with remaining elements
  plot <- plot +
    geom_point(size = 3, aes(colour = endpoint), show.legend = FALSE) +
    geom_errorbarh(aes(xmin = lowerCI, xmax = upperCI, colour = endpoint), 
                  height = 0.3, size = 0.5, show.legend = FALSE) +
    # Add ND labels for missing values
    geom_text(aes(label = ifelse(missingvalues == "ND", "ND", "")), 
              x = ifelse(is.na(min_x), min(df$lowerCI, na.rm = TRUE) * 0.9, min_x), 
              hjust = -0.5, vjust = 0.5)
  
  # Add warning as caption instead of annotation
  if (!is.null(warning_text)) {
    plot <- plot + labs(
      title = paste0(unique(df$chemical), " (n=", length(logBMDvalues[[1]]), " genes)"),
      x = paste0("logBMD ", "(", unique(metadata$DoseUnits), ")"),
      y = "Endpoint",
    ...
    theme(
      ...
      plot.caption = element_text(color = "red", face = "bold", hjust = 0.5, size = 10)
    )
  } else {
    plot <- plot + labs(
      title = paste0(unique(df$chemical), " (n=", length(logBMDvalues[[1]]), " genes)"),
...
# Call the function with all necessary parameters
summary_plot <- create_summary_plot(
  df = bigtibble,
  params = params,
  chemname = chemname,
  lowestdoses = lowestdoses,
  highestdoses = highestdoses,
  metadata = metadata,
  logBMDvalues = logBMDvalues,
  goBMDvalues = goBMDvalues,
  reactomeBMDvalues = reactomeBMDvalues
)
